...
# Outlier removal (get rid of obs with a value more than threshold standard deviations away from mean)
#threshold = 5
#df[(np.abs(stats.zscore(df)) < threshold).all(axis=1)]


# shuffles to break up any inherent order the observations might have
df = df.sample(frac=1, random_state=0) # important for k-fold crossvalidation


# make regressors
y, X = df.status.values, df.drop(columns=['NSP','status']).values # make regressors

###########################################################
### Feature engineering

def lof_outlier_removal(X,share_out=0.01):
    ''' Removes outliers from X using local outlier factors. Number of removed 
    outliers based on share_out (prior on / desired share of outliers). '''
...
# Make outlier_removal a transformer function
outlier_removal = sklearn.preprocessing.FunctionTransformer(lof_outlier_removal)
...
processing = sklearn.pipeline.Pipeline([('outlier', outlier_removal), ('poly', poly), 
...
log_reg_cv = sklearn.linear_model.LogisticRegressionCV(Cs=10, fit_intercept=True, 
   cv=k_fold_strat_oversamp(X,y,k=10), dual=False, penalty='elasticnet', 
   scoring='balanced_accuracy', solver='saga', tol=0.0001, max_iter=10000, 
...
log.steps.append(['log_reg', log_reg_cv])
log.steps
processing.steps
